  ...
  try {
    const response = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        ...
        to: [email],
        subject: 'Information importante — assistant BacPilot temporairement indisponible',
        html: incidentHtml(displayName),
...
Deno.serve(async (request) => {
  ...
  for (const profile of (profiles || []) as Profile[]) {
    ...
    const result = await sendEmail(resendKey, email, clean(profile.display_name, 120));
