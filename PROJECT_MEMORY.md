...
| Dernier commit confirmé | état à versionner — webhook Telegram et notification d’inscription configurés |
...
| `notify-new-user` | Reçoit exclusivement un évènement d’insertion de profil autorisé et transmet une alerte d’inscription via Telegram. Un trigger `bacpilot_notify_new_profile` sur `public.profiles` utilise `pg_net` après l’écriture. | Vérifie un secret de webhook conservé dans Vault, détermine le statut bêta côté serveur, journalise l’envoi de façon idempotente ; aucun secret ne figure dans le code, la migration ou la mémoire. |
...
|---:|---|---|
| 1 | Réaliser la recette réelle de la console Telegram et d’une alerte d’inscription. | Le webhook Telegram est enregistré et le trigger de profils est actif ; valider maintenant `/help`, `/user`, une confirmation bêta et une alerte unique lors de la prochaine vraie inscription. |
