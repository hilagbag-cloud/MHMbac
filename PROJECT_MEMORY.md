...
| Dernier commit confirmé | `f31850e` — création de compte standard explicitement accessible depuis le portail bêta |
...
| 15 août 2026 | En attente de commit et de déploiement | Diagnostic d’un crash sur `/register` et `/register?returnTo=beta` : l’import différé attendait un export par défaut alors que `RegisterPage` est un export nommé. `App.tsx` charge maintenant explicitement `module.RegisterPage`. Compilation Vite et contrôle de diff réussis. | Déployer et tester les deux variantes de l’inscription ainsi que le flux bêta. |
