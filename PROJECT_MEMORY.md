...
| Dernier commit confirmé | `34513ac` — confirmation des contrôles de production du portail bêta |
...
| 15 août 2026 | En attente de commit et de déploiement | Le portail bêta propose explicitement « Créer mon compte » vers l’inscription principale avec l’intention `returnTo=beta`, ainsi que « J’ai déjà un compte ». Toute personne peut donc créer un compte BacPilot ; seules les fonctions de bêta-test exigent une validation serveur dans `beta_testers`. Compilation Vite et contrôle de diff réussis. | Publier la modification, puis configurer le canal de notification privé de l’opérateur. |
