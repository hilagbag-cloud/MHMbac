...
## Mise à jour du 17 août 2026 — résilience multi-fournisseurs IA

L’écran « L’assistant est momentanément indisponible » a été diagnostiqué. Les journaux PostgreSQL ont montré `permission denied for table guide_programmes` après l’installation du RPC personnalisé, ainsi qu’une violation `profiles.display_name NOT NULL` lors d’un enregistrement partiel. La fonction `get_personalized_recommendations(...)` est maintenant `SECURITY DEFINER` avec `search_path = public, extensions`, sans droit direct exposé aux clients. `orientation-assistant` met à jour un profil existant au lieu de tenter un upsert incomplet ; l’insertion de repli utilise uniquement le nom du profil ou « Candidat ».

La fonction `orientation-assistant` version 27 est active. Elle contient maintenant une chaîne configurable `AI_PROVIDER_ORDER` (par défaut `gemini,groq,openrouter,cerebras`). Gemini reste prioritaire, Groq est le premier repli, OpenRouter et Cerebras sont optionnels et ne sont appelés que si leur secret est présent. Chaque fournisseur a un seul appel par requête, un timeout de 8 secondes et aucun mécanisme de rotation destiné à contourner les limites. Le fournisseur effectivement utilisé est renvoyé sous `ai_provider`, sans exposer les clés.
...
## Recommandations IA qualitatives — correctif vérifié (17 août 2026)

Le comportement répétitif venait d’une combinaison de trois mécanismes : le RPC historique préclassait les mêmes filières avec un score numérique, l’IA ne faisait qu’un réordonnancement minimal de ce Top 12 et toute sortie JSON imparfaite retombait silencieusement sur les trois premières lignes SQL. Le texte déterministe affichait alors « score indicatif », ce qui donnait l’impression exacte d’un modèle préfabriqué.

Le contrat actif utilise maintenant `get_personalized_candidate_pool`, un pool de six à dix-huit filières réellement observées, construit à partir de compatibilité de série attestée par le guide, du domaine déclaré, des forces académiques, des quotas et de la pression observés. Le RPC ne retourne aucun score exposable. La fonction `orientation-assistant` v38 demande ensuite au fournisseur IA un JSON structuré avec trois `programme_id` autorisés, une synthèse personnalisée et une raison qualitative par filière. Toute sortie invalide, tout épuisement de quota ou tout échec de fournisseur affiche un état honnête sans Top 3 préfabriqué.
...
Le contrôle public final après publication a confirmé que la page fondateur rend bien `ProfilePage`, `dateModified: 2026-08-18T00:00:00+01:00`, l’identifiant Person stable et le profil LinkedIn canonique validé.


## Mise à jour du 19 août 2026 — fiches individuelles volontaires des contributeurs bêta

Le pilote de fiches individuelles est publié sur `https://bacpilot.site`. Il complète l’annuaire `/contributeurs-beta` sans créer ni publier de nouvelle fiche de lui-même. Une fiche individuelle n’existe que lorsqu’un bêta-testeur actif choisit `Fiche personnelle`, renseigne une présentation d’au moins 60 caractères, propose un slug public, accepte séparément la publication et l’exploration par les moteurs, puis atteste disposer de l’autorisation nécessaire. L’attestation prévoit explicitement l’accord du responsable légal lorsque la personne est mineure, sans stocker de date de naissance ni de justificatif.

Les migrations appliquées sont `20260819_bacpilot_contributor_public_profiles_pilot.sql`, `20260819_bacpilot_contributor_public_profile_attestation.sql` et `20260819_bacpilot_contributor_public_profile_sitemap.sql`. Elles ajoutent `publication_status`, `public_slug`, dates de publication/retrait, attestation et fonctions serveur bornées. Les fonctions SQL de profil et de sitemap sont exécutables uniquement par `service_role` : les rôles `anon` et `authenticated` ne peuvent pas les appeler directement.

La fonction Edge `public-beta-contributor-profile` est active en version 2 et sert les fiches HTML avec canonique, `ProfilePage` JSON-LD, fil d’Ariane et une photo stable seulement si la photo est explicitement consentie. La fonction `public-beta-contributors` est active en version 2 et ne propose une URL individuelle que pour un profil `published_profile`. Le bucket photo reste privé. La nouvelle route `https://bacpilot.site/sitemap-contributeurs-beta.xml` est déclarée dans `robots.txt` et ne contient que les fiches individuelles actives et consenties ; elle est actuellement vide. La route d’un slug inexistant répond `404` avec `X-Robots-Tag: noindex, nofollow` ; un profil retiré répondra `410` avec la même directive, après retrait immédiat de l’annuaire et du sitemap.
...
## Mise à jour du 20 août 2026 — sécurité e-mail, avis privé et suppression opérateur
Une migration de sécurité `20260820_bacpilot_account_notice_and_email_integrity.sql` est appliquée en production. Elle ajoute aux profils `account_status` (`active` ou `suspended_notice`) et un avis individuel (titre, texte, motif et date). Le statut `suspended_notice` ne supprime aucune donnée : il bloque les parcours applicatifs et montre à la prochaine connexion un écran de vérification visible uniquement par le titulaire. Cette branche est obligatoire lorsque l’adresse est absente, malformée, ne correspond pas à l’identité Auth ou n’est pas confirmée. Une suppression définitive ne pourrait pas afficher de message dans une session qui n’existe plus ; ne pas confondre ces deux opérations.

L’écran `/register` normalise désormais l’adresse et refuse les formats manifestement malformés. Le profil est créé par un déclencheur `auth.users` côté serveur ; cette création reste donc disponible lorsque la confirmation d’adresse empêche l’ouverture immédiate d’une session. L’e-mail de profil est verrouillé à l’identité Auth et ne peut plus être changé directement par le client. L’interface affiche explicitement « Confirme ton adresse e-mail » si Auth impose cette étape.

Le bot Telegram `bacpilot-telegram` est actif en version **49**. Dans toute fiche utilisateur et dans la notification d’une nouvelle inscription, le bouton **« Vérifier / supprimer »** ouvre un choix de motif : informations non vérifiables, doublon, règles, demande du titulaire ou texte personnalisé. Le bot prépare toujours un brouillon et demande une confirmation. Si l’e-mail est admissible, l’action envoie un e-mail de suppression avec le motif, journalise la demande, utilise une clé d’idempotence Resend, puis supprime le compte seulement si l’envoi est accepté et le journal enregistré. Si l’e-mail n’est pas fiable, aucune suppression ni aucun e-mail ne part : le bot prépare une restriction avec avis privé et demande confirmation. Les actions expirent après 10 minutes. Aucun e-mail, aucune restriction et aucune suppression n’ont été exécutés pendant le déploiement.

La fonction `notify-new-user` est active en version **24**. Elle affiche dans Telegram la qualité d’adresse (format et confirmation Auth), n’envoie pas de welcome à une adresse manifestement absente/malformée et expose le nouveau bouton de vérification/suppression. Les contrôles non destructifs ont confirmé que le format valide est accepté, le format malformé refusé et que les **37** profils existants restent tous actifs, sans suspension involontaire. L’interface publiée est vérifiée sur `https://bacpilot.site/register`.

> Action opérateur manuelle prioritaire : activer **Authentication → Providers → Email → Confirm email** dans Supabase, puis vérifier `https://bacpilot.site` et `https://bacpilot.site/**` dans les URL de redirection. L’audit agrégé précédent indiquait que 35 comptes sur 36 étaient marqués confirmés dans la minute, ce qui signale que la confirmation Auth n’était pas exigée. Cette action change la politique de connexion globale et ne doit pas être appliquée silencieusement. Le détail opérateur est documenté dans `docs/SECURITE_SUPPRESSION_COMPTE_EMAIL_20260820.md`.

## Mise à jour du 20 août 2026 — campagne de retour orientation
La campagne de retour des utilisateurs est déployée dans `bacpilot-telegram` version **51**, sous la commande `/orientation_return_draft` et le bouton inline **« Campagne de retour »**. Elle ne contient aucune date de clôture inventée : elle indique uniquement que la période de choix approche de sa clôture et que BacPilot reste disponible pour aider à reprendre le profil, comparer les pistes, relire les informations et préciser les objectifs. Le message rappelle que les résultats sont indicatifs et que les décisions doivent être vérifiées sur les plateformes officielles. Son objet est : « La période de choix avance : BacPilot reste à vos côtés » ; le CTA HTML mène à `https://bacpilot.site/onboarding`.

L’audience est volontairement limitée aux comptes `active` avec e-mail de format valide, identique dans Auth et profil, et `email_confirmed_at` présent dans Auth. L’audit agrégé avant livraison a trouvé 36 profils actifs, dont 31 avec adresse au format valide et Auth confirmée, et aucun compte restreint. Les adresses individuelles ne sont pas exposées au bot dans l’aperçu. La campagne crée un brouillon de dix minutes, affiche le message complet et exige la confirmation explicite de l’opérateur. Au moment de cette confirmation, l’audience est revalidée ; les comptes devenus inactifs ou non confirmés sont exclus. Chaque demande vers Resend utilise une clé d’idempotence `orientation-return/<action>/<user>` et une ligne de journal par destinataire. L’action `email_campaign_orientation_return` a été ajoutée par la migration `20260820_bacpilot_orientation_return_campaign.sql`. Aucun e-mail, brouillon en attente, compte ou statut n’a été créé/modifié pendant le déploiement. Le détail est dans `docs/CAMPAGNE_RETOUR_ORIENTATION_20260820.md`.

### Personnalisation de la campagne de retour — 20 août 2026
La campagne `/orientation_return_draft` a été enrichie et le bot `bacpilot-telegram` est en version **52**. Le message n’est plus fixe : le parcours Telegram demande successivement 1) le nombre de jours avant clôture, 2) le sujet ou `OK` pour le modèle proposé, 3) le texte complet ou `OK` pour le modèle proposé. Exemple pour `4` jours : « Plus que 4 jours avant la clôture des choix : BacPilot reste à vos côtés ». Seul l’aperçu final crée un brouillon en attente de dix minutes ; il affiche ensuite Confirmer / Annuler. Les étapes de saisie ne créent aucune campagne et n’envoient aucun e-mail. La migration `20260820_bacpilot_orientation_return_composer.sql` étend de façon minimale les états et commandes de session Telegram. Après confirmation seulement, l’envoi utilise le sujet et le texte explicitement choisis par l’opérateur et conserve le filtre d’audience active/Auth-confirmée ainsi que l’idempotence par destinataire. Vérification après publication : 0 brouillon de retour en attente et 0 livraison de cette campagne. Documentation mise à jour : `docs/CAMPAGNE_RETOUR_ORIENTATION_20260820.md`.

### Correctif validation e-mail mobile — 20 août 2026
Le champ e-mail de `src/pages/RegisterPage.tsx` contenait une contrainte HTML `pattern` redondante, distincte de la validation applicative `EMAIL_FORMAT`. Sur un navigateur mobile, elle bloquait une adresse pourtant correcte comme `molihhior@gmail.com` avec le message natif « Veuillez respecter le format requis ». Le `pattern` a été retiré. Le champ conserve `type="email"` et l’application conserve la validation `EMAIL_FORMAT` (`^[^\s@]+@[^\s@]+\.[^\s@]+$`) avant l’appel Auth, puis Supabase doit confirmer la possession réelle de l’adresse lorsque l’option Confirm email est activée. Production publiée sur Vercel ; test DOM production : l’adresse Gmail de signalement et une adresse avec `+` sont acceptées, le champ ne porte plus de pattern. Commit GitHub : `63eeef8cf3afc51ffe998c6345d30b20a47fe1da`.
