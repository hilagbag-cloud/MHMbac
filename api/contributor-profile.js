...
function unavailablePage(status, title, message) {
...
export default async function handler(req, res) {
  ...
  if (!SLUG_PATTERN.test(slug)) {
    noIndex();
    res.status(404).send(unavailablePage(404, 'Profil introuvable', 'Cette fiche contributeur n’existe pas ou n’est pas disponible publiquement.'));
    return;
  }
  ...
  try {
    ...
    if (!upstream.ok || !payload?.profile) {
      ...
      noIndex();
      res.status(status).send(unavailablePage(status, status === 410 ? 'Profil retiré' : 'Profil introuvable', payload?.error?.message || 'Cette fiche contributeur n’est pas disponible publiquement.'));
      return;
    }
    html();
    ...
    noIndex();
    res.status(503).send(unavailablePage(503, 'Profil momentanément indisponible', 'Cette fiche ne peut pas être affichée pour le moment.'));
  }
}
