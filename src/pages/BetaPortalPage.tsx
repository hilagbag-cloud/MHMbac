...
interface BetaPortalPageProps { accessUrl: string; signUpUrl: string; }

export const BetaPortalPage: React.FC<BetaPortalPageProps> = ({ accessUrl, signUpUrl }) => {
  const start = () => { window.location.assign(accessUrl); };
  const signUp = () => { window.location.assign(signUpUrl); };
  return <main className="min-h-screen bg-slate-950 text-white"><section className="mx-auto flex min-h-screen max-w-6xl flex-col justify-center px-5 py-16 sm:px-8"><div className="mb-8 inline-flex w-fit items-center gap-2 rounded-full border border-rose-400/30 bg-rose-500/10 px-3 py-1.5 text-xs font-black uppercase tracking-[0.18em] text-rose-200"><FlaskConical className="h-4 w-4" /> Programme bêta BacPilot</div><div className="grid gap-12 lg:grid-cols-[1.1fr_0.9fr] lg:items-center"><div><h1 className="max-w-3xl text-4xl font-black tracking-tight sm:text-6xl">Teste BacPilot.<br /><span className="text-rose-400">Améliore l’orientation.</span></h1><p className="mt-6 max-w-2xl text-base leading-7 text-slate-300 sm:text-lg">Tu peux créer un compte BacPilot depuis ce portail et accéder ensuite à la plateforme. Les outils de bêta-test — signalement, captures et suivi de test — s’activent seulement après validation de ton invitation par l’équipe BacPilot.</p><div className="mt-8 flex flex-col gap-3 sm:flex-row"><button onClick={signUp} className="inline-flex items-center justify-center gap-2 rounded-xl bg-rose-500 px-6 py-3.5 font-black text-white transition hover:bg-rose-400">Créer mon compte <ArrowRight className="h-4 w-4" /></button><button onClick={start} className="inline-flex items-center justify-center rounded-xl border border-slate-700 px-6 py-3.5 font-bold text-slate-200">J’ai déjà un compte</button></div><p className="mt-4 text-xs leading-5 text-slate-500">La vérification d’identité et de statut se déroule sur `bacpilot.site`. Aucun jeton ou rôle n’est placé dans le lien.</p></div><div className="rounded-3xl border border-slate-800 bg-slate-900/70 p-6 sm:p-8"><h2 className="text-xl font-black">Ton rôle de bêta-testeur</h2><div className="mt-6 space-y-5">{[
