...
export const BetaAccessPage: React.FC<BetaAccessPageProps> = ({ navigate }) => {
  ...
  return <main className="flex min-h-[70vh] items-center justify-center bg-slate-50 px-4 py-12 dark:bg-slate-950"><section className="w-full max-w-2xl rounded-3xl border border-amber-200 bg-white p-8 text-center shadow-sm dark:border-amber-900/50 dark:bg-slate-900 sm:p-10"><div className="mx-auto flex h-14 w-14 items-center justify-center rounded-2xl bg-amber-50 text-amber-500 dark:bg-amber-950/30"><CircleAlert className="h-7 w-7" /></div><h1 className="mt-5 text-2xl font-black">Invitation bêta requise</h1><p className="mx-auto mt-3 max-w-xl text-sm leading-6 text-slate-500">Ton compte est bien connecté, mais il n’est pas encore actif dans le programme bêta. L’équipe BacPilot doit d’abord l’enrôler côté serveur ; ce statut ne peut pas être choisi depuis le navigateur.</p><div className="mt-7 flex flex-col justify-center gap-3 sm:flex-row"><button onClick={() => navigate('/dashboard')} className="rounded-xl border border-slate-300 px-5 py-3 text-sm font-bold dark:border-slate-700">Retour au tableau de bord</button><button onClick={() => navigate('/contact')} className="inline-flex items-center justify-center gap-2 rounded-xl bg-rose-500 px-5 py-3 text-sm font-bold text-white"><ShieldCheck className="h-4 w-4" /> Demander une invitation</button></div><p className="mt-6 text-xs text-slate-400">Compte connecté : {user.email}</p></section></main>;
};
