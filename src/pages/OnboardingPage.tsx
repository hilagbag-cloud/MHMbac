  ...
  return <main className="min-h-[70vh] bg-slate-50 dark:bg-slate-950 px-4 py-10 text-slate-900 dark:text-white">
    <div className="mx-auto max-w-2xl">
      ...
      <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900 sm:p-10">
        ...
        <div hidden={step !== 0}>
          <input autoFocus={step === 0} value={displayName} onChange={(event) => setDisplayName(event.target.value)} placeholder="Ex. Hilarus Gbagoule" className="w-full rounded-xl border border-slate-300 bg-transparent px-4 py-3 outline-none focus:border-rose-500 dark:border-slate-700" />
        </div>
        <div hidden={step !== 1} className="grid grid-cols-2 gap-3 sm:grid-cols-3">
          {seriesOptions.map((option) => <button type="button" key={option.value} onClick={() => setSeries(option.value)} className={`rounded-xl border px-4 py-4 text-left font-semibold ${series === option.value ? 'border-rose-500 bg-rose-50 text-rose-700 dark:bg-rose-950/30 dark:text-rose-300' : 'border-slate-200 dark:border-slate-700'}`}>{series === option.value && <Check className="mb-2 h-4 w-4" />}{option.label}</button>)}
        </div>
        <div hidden={step !== 2} className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          {mentionOptions.map((option) => <button type="button" key={option} onClick={() => setMention(option)} className={`rounded-xl border px-4 py-4 text-left font-semibold ${mention === option ? 'border-rose-500 bg-rose-50 text-rose-700 dark:bg-rose-950/30 dark:text-rose-300' : 'border-slate-200 dark:border-slate-700'}`}>{mention === option && <Check className="mb-2 h-4 w-4" />}{option}</button>)}
        </div>
        <div hidden={step !== 3} className="grid gap-3 sm:grid-cols-2">
          <button type="button" onClick={() => setGoal('bourse')} className={`rounded-xl border p-5 text-left ${goal === 'bourse' ? 'border-emerald-500 bg-emerald-50 dark:bg-emerald-950/30' : 'border-slate-200 dark:border-slate-700'}`}><strong>Classement selon mes chances de bourse</strong><p className="mt-2 text-sm text-slate-500">Priorise les bourses observées et la concurrence mesurée.</p></button>
          <button type="button" onClick={() => setGoal('carriere')} className={`rounded-xl border p-5 text-left ${goal === 'carriere' ? 'border-rose-500 bg-rose-50 dark:bg-rose-950/30' : 'border-slate-200 dark:border-slate-700'}`}><strong>Classement selon mon objectif carrière</strong><p className="mt-2 text-sm text-slate-500">Met en avant les données live, puis ton domaine d’intérêt.</p></button>
        </div>
        <div hidden={step !== 4}>
          <input autoFocus={step === 4} value={career} onChange={(event) => setCareer(event.target.value)} placeholder={goal === 'carriere' ? 'Ex. informatique, santé, agronomie' : 'Facultatif pour la bourse'} className="w-full rounded-xl border border-slate-300 bg-transparent px-4 py-3 outline-none focus:border-rose-500 dark:border-slate-700" />
        </div>
