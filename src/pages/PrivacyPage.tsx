    ...
    <div className="min-h-screen bg-slate-50 py-12 text-slate-900 dark:bg-slate-950 dark:text-white sm:py-16">
      <div className="mx-auto max-w-4xl space-y-8 px-4 sm:px-6 lg:px-8">
        ...
        <div className="space-y-6 text-sm leading-7 text-slate-600 dark:text-slate-300">
          ...
          <section className="rounded-3xl border border-slate-200 bg-white p-7 dark:border-slate-800 dark:bg-slate-900"><ShieldCheck className="h-5 w-5 text-emerald-500" /><h2 className="mt-4 text-xl font-black text-slate-900 dark:text-white">Accès et protection</h2><p className="mt-3">Les profils et préférences sont protégés par des règles d’accès : un utilisateur connecté accède uniquement à ses propres données. Les observations publiques utilisées pour comparer les filières sont séparées des données de profil.</p><p className="mt-3">Si vous demandez volontairement à devenir bêta-testeur, BacPilot demande votre accord avant de transmettre à l’équipe le point d’entrée, la catégorie de terminal et la famille de navigateur, afin de traiter cette demande. Aucun mot de passe, jeton, cookie, adresse IP, session brute ou contenu privé n’est transmis dans la notification opérateur.</p></section>

          <section className="rounded-3xl border border-slate-200 bg-white p-7 dark:border-slate-800 dark:bg-slate-900"><Eye className="h-5 w-5 text-amber-500" /><h2 className="mt-4 text-xl font-black text-slate-900 dark:text-white">Reconnaissance des bêta-testeurs</h2><p className="mt-3">La reconnaissance des bêta-testeurs est facultative. Par défaut, le suivi de contribution, les retours, les captures et les informations de profil restent privés. Un bêta-testeur peut choisir séparément son nom public, une courte présentation, des liens professionnels et une photo. L’e-mail, les données scolaires, les détails des retours, les informations de session et les captures de bugs ne sont jamais publiés sur la page des contributeurs.</p><p className="mt-3">Un profil ne devient visible que si son titulaire active volontairement la visibilité publique et accepte qu’il puisse être exploré par les moteurs de recherche. Une fiche individuelle à adresse stable nécessite en plus une courte présentation et une attestation que la personne dispose de l’autorisation nécessaire pour publier son nom, ses liens ou sa photo. La photo nécessite un accord distinct.</p><p className="mt-3">Ces réglages peuvent être retirés à tout moment depuis l’espace bêta. BacPilot retire alors la fiche de l’annuaire et du sitemap immédiatement, puis renvoie une réponse non indexable pour aider les moteurs à constater le retrait. Une copie éventuellement déjà enregistrée par un moteur ou un tiers peut toutefois disparaître selon son propre délai de mise à jour.</p></section>
