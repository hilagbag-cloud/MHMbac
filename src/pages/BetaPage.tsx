...
export const BetaPage: React.FC<BetaPageProps> = ({ navigate }) => {
  ...
  if (!user) {
    return <section className="mx-auto flex min-h-[70vh] max-w-3xl items-center px-4 py-16"><div className="w-full rounded-3xl border border-slate-200 bg-white p-8 text-center shadow-sm dark:border-slate-800 dark:bg-slate-900"><ShieldCheck className="mx-auto h-10 w-10 text-rose-500" /><h1 className="mt-4 text-2xl font-black">Espace bêta-testeur</h1><p className="mx-auto mt-3 max-w-xl text-slate-500">Connecte-toi pour accéder aux outils de retour BacPilot.</p><button onClick={() => navigate('/login')} className="mt-6 rounded-xl bg-rose-500 px-5 py-3 font-bold text-white">Se connecter</button></div></section>;
  }
