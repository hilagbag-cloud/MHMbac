    ...
    return (
      <main className="flex min-h-screen items-center justify-center bg-slate-950 px-4 text-white">
        <section className="w-full max-w-lg rounded-3xl border border-slate-800 bg-slate-900 p-8 text-center shadow-2xl">
          ...
          {Boolean((import.meta as ImportMeta & { env?: { DEV?: boolean } }).env?.DEV) && this.state.message && <pre className="mt-4 overflow-auto rounded-xl bg-slate-950 p-3 text-left text-xs text-rose-300">{this.state.message}</pre>}
