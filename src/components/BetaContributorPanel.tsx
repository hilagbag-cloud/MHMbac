...
import { Award, Camera, CheckCircle2, ExternalLink, Globe2, LoaderCircle, ShieldCheck, Sparkles, UserRound } from 'lucide-react';
...
const visibilityOptions: Array<{ value: ContributorVisibility; title: string; text: string }> = [
  { value: 'private', title: 'Privé', text: 'Tu suis ta contribution, mais rien n’apparaît publiquement.' },
  { value: 'name_only', title: 'Nom et niveau', text: 'Ton nom choisi et ton niveau de contribution peuvent apparaître dans la communauté.' },
  { value: 'profile', title: 'Profil contributeur', text: 'Tu peux présenter ta contribution, tes centres d’intérêt et, si tu le souhaites, tes liens publics.' },
];
...
export const BetaContributorPanel: React.FC<{ navigate: (route: string) => void; refreshKey?: number }> = ({ navigate, refreshKey = 0 }) => {
  ...
  const isPublic = visibilityLevel !== 'private';
  const isCompleteProfile = visibilityLevel === 'profile';
  ...
  const load = async () => {
    ...
    try {
      ...
      if (contributor) {
        ...
        setVisibilityLevel(contributor.visibility_level);
  ...
  const save = async () => {
    ...
    try {
      ...
      setMessage({ type: 'success', text: isPublic ? 'Ton profil contributeur est enregistré. Tu gardes le contrôle sur sa visibilité à tout moment.' : 'Tes préférences restent privées. Ton indicateur de contribution continue à se mettre à jour.' });
      await load();
    } catch (error: any) {
  ...
  return <section className="space-y-6" aria-labelledby="contribution-title">
    ...
    <div className="rounded-3xl border border-slate-200 bg-white p-6 dark:border-slate-800 dark:bg-slate-900 sm:p-8"><div className="flex items-start gap-3"><div className="rounded-2xl bg-rose-50 p-3 text-rose-600 dark:bg-rose-950/30 dark:text-rose-300"><UserRound className="h-5 w-5" /></div><div><h2 className="text-xl font-black">Mon profil contributeur</h2><p className="mt-1 text-sm leading-6 text-slate-500">Tu décides si BacPilot peut citer ta participation. Rien n’est publié sans ton accord explicite.</p></div></div>
      ...
      <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:items-center"><button type="button" disabled={saving} onClick={() => void save()} className="inline-flex items-center justify-center gap-2 rounded-xl bg-rose-500 px-5 py-3 font-black text-white disabled:cursor-not-allowed disabled:opacity-60">{saving ? <LoaderCircle className="h-4 w-4 animate-spin" /> : <Sparkles className="h-4 w-4" />}{saving ? 'Enregistrement…' : 'Enregistrer mes choix'}</button>{isPublic && <button type="button" onClick={() => navigate('/contributeurs-beta')} className="inline-flex items-center justify-center gap-2 rounded-xl border border-slate-300 px-5 py-3 text-sm font-bold dark:border-slate-700"><ExternalLink className="h-4 w-4" /> Consulter la page publique</button>}</div>
    </div>
  </section>;
};
